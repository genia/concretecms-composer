...
<div class="store-product-list-block">
    ...
    <div class="store-product-list row store-product-list-per-row-<?= $productsPerRow ?>">
        ...
        foreach ($products as $product) {
            ...
            $outOfStockClass = !$isSellable ? 'store-product-out-of-stock' : '';
            $productPage = $product->getProductPage();
            // Always get product page URL if it exists, regardless of showPageLink setting
            $productPageUrl = $productPage ? h((string) $urlResolver->resolve([$productPage])) : '';
            ?>
            <div class="store-product-list-item mb-3 <?= $columnClass ?> <?= $activeclass ?> <?= $outOfStockClass ?>" 
                ...
                <form data-product-id="<?= $product->getID() ?>">
                    ...
                    // Prepare price HTML for overlay on image
                    $priceHtml = '';
                                ...
                                <a href="<?= h((string) $urlResolver->resolve([$productPage])) ?>">
                                    ...
                                    <?php if (!$isSellable): ?>
                                        <div class="store-out-of-stock-overlay">
                                            <?php
                                            $outOfStockMessage = $csm->t($product->getOutOfStockMessage(), 'productOutOfStockMessage', $product->getID());
                                            echo h($outOfStockMessage ?: t('Out of Stock'));
                                            ?>
                                        </div>
                                    <?php endif; ?>
                                </a>
                    ...
                    if ($showPageLink && $productPage || $showAddToCart) {
                        ?>
                    ...
                    if ($showPageLink && $productPage) {
                        ?>
                            <a href="<?= h((string) $urlResolver->resolve([$productPage])) ?>" class="store-btn-more-details store-product-list-block__more-details">
...
$(document).ready(function() {
    // Make product tiles clickable - navigate to product detail page
    $('.store-product-list-item[data-product-url]').on('click', function(e) {
        // Don't navigate if clicking on:
        // - Links (a tags)
        // - Buttons (button tags)
        // - Form inputs (input, select, textarea)
        // - Elements with data-no-navigate attribute
        var $target = $(e.target);
        var $clickedElement = $target.closest('a, button, input, select, textarea, [data-no-navigate], label');
