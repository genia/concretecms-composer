...
<?php } elseif (in_array($controller->getAction(), $listViews)) { ?>
    ...
    <div class="ccm-dashboard-header-buttons">
        <!-- Selection counter -->
        <span class="btn btn-light btn-sm disabled me-2" id="selected-count" style="display: none;">0 <?= t("selected") ?></span>
        ...
        <div class="btn-group ms-2">
            <button type="button" class="btn btn-secondary dropdown-toggle" data-bs-toggle="dropdown" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false" title="<?= t('Bulk Actions') ?>">
                <i class="fa fa-bars"></i>
            </button>
            <ul class="dropdown-menu dropdown-menu-end">
                <li><a class="dropdown-item text-danger" href="#" id="delete-selected-btn"><i class="fa fa-trash"></i> <?= t("Delete Selected") ?></a></li>
            </ul>
        </div>
    ...
    <div class="cccm-dashboard-content-inner">
        ...
        <div class="ccm-dashboard-content-full">
            ...
            <table class="ccm-search-results-table" id="products-table">
            <thead>
            <tr>
                <th style="width: 40px;"><input type="checkbox" id="select-all-checkbox" title="<?= t('Select All') ?>"></th>
            ...
            <?php if (count($products) > 0) {
                foreach ($products as $product) {
                    ...
                    <tr data-product-id="<?= $product->getID() ?>">
                        <td><input type="checkbox" class="product-checkbox" value="<?= $product->getID() ?>"></td>
    ...
    $(document).ready(function() {
       ...
       $('.btn-clearfile').click(function(e){
        ...
        function updateSelectedCount() {
            var count = $productCheckboxes.filter(':checked').length;
            $selectedCount.text(count + ' <?= t("selected") ?>');
            if (count > 0) {
                $selectedCount.show();
            } else {
                $selectedCount.hide();
        ...
        $productCheckboxes.on('change', function() {
            var allChecked = $productCheckboxes.length === $productCheckboxes.filter(':checked').length;
        ...
        $('#delete-selected-btn').on('click', function(e) {
            ...
            var selectedIds = [];
            $productCheckboxes.filter(':checked').each(function() {
                selectedIds.push($(this).val());
            });
            
            if (selectedIds.length === 0) {
                alert('<?= t("Please select at least one product to delete.") ?>');
                return;
            }
            
            if (!confirm('<?= t("Are you sure you want to delete") ?> ' + selectedIds.length + ' <?= t("product(s)? This action cannot be undone.") ?>')) {
                return;
            }
            
            // Send AJAX request to delete
            $.ajax({
                url: '<?= Url::to("/dashboard/store/products/deleteSelected") ?>',
                method: 'POST',
                data: {
                    productIds: selectedIds,
                    ccm_token: '<?= $token->generate("community_store_bulk") ?>'
                },
                dataType: 'json',
                success: function(response) {
                    if (response.success) {
                        ...
                        selectedIds.forEach(function(id) {
                            $('tr[data-product-id="' + id + '"]').fadeOut(300, function() {
                                $(this).remove();
                                updateSelectedCount();
                            });
                        });
                        ...
                        // Reload page if all products deleted
                        if ($productCheckboxes.length === selectedIds.length) {
                            location.reload();
                        }
                    } else {
                ...
                error: function(xhr) {
                    var msg = '<?= t("An error occurred while deleting products.") ?>';
                    try {
...
<style>
    ...
    tr[data-product-id] td:first-child {
